...
proc createSwitches {netw clk swNames} {
  ...
  foreach swName $swNames {
    set sw [create_object -type "switch" -parent $netw -name $swName]
...
proc trimSwitches {wid clk netw pp1 sw1 sw2} {
  ...
  if {$pps1_sw == $sw1 && $pps2_sw == $sw2} {
    ...
    set wAdap1 [create_object -type width_adapter -name $nam -parent $netw]
    update_object -name $wAdap1 -bind $clk -type "domain"
    insert_adapter -port $pps1 -name $wAdap1
    ...
    set wAdap2 [create_object -type width_adapter -name $nam -parent $netw]
    update_object -name $wAdap2 -bind $clk -type "domain"
    insert_adapter -port $pps2 -name $wAdap2
...
proc run_dv_gen_collateral {} {
  ...
  if {[info exists $env(USE_MAESTRO_SERVER)]} {
...
proc debug_continue_on_errors { val } {
  global _debug_continue_on_errors
  set _debug_continue_on_errors $val
}
...
set base_config_dir $curDir
puts "base_config = $base_config"
puts "base_config_dir = $base_config_dir"
...
set routing_auto_create_node_locs 1 ; # If 0, user specify node locations. If 1, auto create node locations from script.
if {[info exists _debug_continue_on_errors] == 0 } {
  set _debug_continue_on_errors 0 ; # If 1, allow errors to continue. If 0, stop on error
  puts "# ### Defaulting _debug_continue_on_errors = $_debug_continue_on_errors"
}
if {[info exists arteris_license] == 0} {
  set arteris_license "Arteris/Dev"
  puts "# ### Defaulting arteris_license = $arteris_license"
}
set switchBufferDepth 2
#### Global parameters


puts "Design Name: $designName license=$arteris_license"
set project [create_project -name $designName -license_token $arteris_license]
...
set myProc [info procs base_safetyParams]
if {$myProc != ""} {
  base_safetyParams $subsystem
}

set myProc [info procs mut_safetyParams]
if {$myProc != ""} {
  mut_safetyParams $subsystem
}
...
# All designs have a mandatory APB debug Socket (MAES-5841)
set debug_socket [create_object -parent $subsystem -type socket -name "debug_apb"]
set_attribute -object $debug_socket -name "socketFunction" -value "EXTERNAL_DEBUG"
update_object -name $debug_socket -bind [Socket::get_debugApbSocketClock $subsystem] -type domain
...
if {$_debug_continue_on_errors == 1} {
  if {[info proc debug_stop_on_errors] != ""} {
    puts "# Warning: debug_continue_on_errors set to allow errors to continue"
    debug_stop_on_errors -bypass
  } else {
    puts "# Warning: debug_continue_on_errors set to stop on errors"
  }
} else {
    puts "# Warning: Defaulting debug_continue_on_errors set to stop on errors"
}
...
set caius [get_objects -type caiu -parent $chip]

foreach caiu $caius {
  set clk [Caiu::get_clock $caiu]
  set ts_clk [Ts::get_interconnect_clock]
  set freq [Clock::get_freq_from_subdomain $caiu]
  if {$freq > 1600000} {
     Caiu::set_clock $caiu [Ts::get_interconnect_clock]
     puts "The caiu $caiu has freq greater than 1.6GHz. Hence inserting a chi_async_adapter"
  }
}

#Assign clock to the GRB unit
set grb [get_objects -type grb -parent $chip]
update_object -name $grb -type "domain" -bind [Ts::get_interconnect_clock]
...
if {$userAssignLocation == 1} {
  set routing_auto_create_node_locs 0
  ...
  set params [list meshx $meshX meshy $meshY]
} else {
  set routing_auto_create_node_locs 1
  set params [Ts::auto_create_node_positions $routing_template]
}
...
if {[string compare $routing_template "manual"] == 0} {
  ...
  lappend params clock $default_clock


  MeshGenerator::route_networks $chip $topology $routing_template $routing_dataWidth $params $switchBufferDepth
}
...
set isDuplicationEnabled "false"
set isResilencyEnabled   "false"

set qResilencyEnabled   [get_parameter -object $subsystem/safetyconfig -name resilienceEnabled]
if {$qResilencyEnabled != "false"} {
  set isResilencyEnabled   [Safety::get_useResiliency $subsystem/safetyconfig]
}

if {$isResilencyEnabled == "true"} {
  set qDuplicationEnabled [get_parameter -object $subsystem/safetyconfig -name duplicationEnabled]
  if {$qDuplicationEnabled != "false"} {
    set isDuplicationEnabled [Safety::get_enableUnitDuplication $subsystem/safetyconfig]
  }

  puts "#### ++++ enableResilience $isResilencyEnabled enableDuplication $isDuplicationEnabled ++++ ####"
  set resil_params [list enableResilience $isResilencyEnabled enableDuplication $isDuplicationEnabled]
  run_generator -topology $topology -clock $default_clock -name "resiliency" -params $resil_params
} else {
  puts "#### ++++ isResilencyEnabled is disabled ++++ ####"
}


#----------------------------------------------------------------------
# Generator for power management
#---------------------------------------------------------------------
