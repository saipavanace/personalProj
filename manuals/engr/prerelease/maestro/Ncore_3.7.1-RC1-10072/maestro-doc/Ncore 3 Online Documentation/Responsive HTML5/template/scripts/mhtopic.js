	...
	this.doHighlight = function( a_aRanges, a_nStart )
	{
		...
		if (gbIE)
		{
		    ...
		    if ((strHTML.length >0)&&(strHTML.charAt(0) == " ")){
		        ...
		        spanElement.insertBefore(nbsp, spanElement.firstChild);
			}
		}
...
const s_strRecursiveTags = "^(sub|sup|img|applet|object|br|iframe|embed|noembed|param|area|input|select|textarea|"+
              "button|option|hr|frame|noframes|marquee|label|p|dl|div|center|noscript|blockquote|form|"+
			  "isindex|table|fieldset|address|layer|dt|dd|caption|thead|tfoot|tbody|tr|th|td|legend|h1|"+
			  "h2|h3|h4|h5|h6|ul|ol|dir|menu|li|pre|xmp|listing|plaintext|ins|del)$";

const s_strSpanInvalidParents = "^(applet|object|iframe|embed|table|thead|tfoot|tbody|tr|ul|ol|script)$";
...
function doesTagRecursiveProcess( a_Node )
{
	...
	var regex = new RegExp(s_strRecursiveTags, "i");
	var ss = regex.exec(strTagName);
	return ss != null;
}
...
function isValidParentForSpan(a_Node)
{
	...
	var strTagName = a_Node.tagName.toLowerCase();
	var regex = new RegExp(s_strSpanInvalidParents, "i");
	var ss = regex.exec(strTagName);
...
{
	var sValue=document.getElementById("searchForm" + nForm).querySelector('[name="searchString"]').value;;
