...
const jsonData = require(process.env.WORK_TOP + '/dv/common/lib_tb/perf_counter/event_list.json');

module.exports = {
  updateRetObj:updateRetObj
}
...
function updateRetObj(obj) {
    ...
    if (obj.BlockId.includes("dce") || obj.BlockId.includes("dve") || obj.BlockId.includes("dmi") || obj.BlockId.includes("dii"))  { AgentInfoName="D"+obj.BlockId.match(/d[a-z]+/i)[0].slice(1)+"Info";} // case dce0,dve1,dmi4 => D + ceInfo ,DveInfo,DmiInfo
...
function listEventPerfCntFunc(obj) {
	...
	if (obj.BlockId.includes("dmi")) { agentStr="dmi";huntAgent=0;} 
	if (huntAgent && obj.BlockId.includes("dii")) { agentStr="dii";huntAgent=0;} 
	if (huntAgent && obj.BlockId.includes("dce")) { agentStr="dce";huntAgent=0;} 
	if (huntAgent && obj.BlockId.includes("dve")) { agentStr="dve";huntAgent=0;} 
    if (huntAgent && (obj.BlockId.includes("ncaiu") || obj.BlockId.includes("ioaiu") || obj.AiuInfo[obj.Id].fnNativeInterface == "AXI4" || obj.AiuInfo[obj.Id].fnNativeInterface == "AXI5" || obj.AiuInfo[obj.Id].fnNativeInterface == "ACE-LITE" || obj.AiuInfo[obj.Id].fnNativeInterface == "ACELITE-E")) {
