...
<%}else{%>
  if(m_constraint_addr  == 1)begin
        FUnitId = addrMgrConst::map_addr2dmi_or_dii(m_ace_rd_addr_chnl_addr,fnmem_region_idx);
...
<%}else{%>
if (m_constraint_addr  == 1)begin
        FUnitId = addrMgrConst::map_addr2dmi_or_dii(m_ace_wr_addr_chnl_addr,fnmem_region_idx);
    if( FUnitId inside {addrMgrConst::dmi_ids}) begin
      unit_type = "DMI";
    end else if( FUnitId inside {addrMgrConst::dii_ids}) begin
      unit_type = "DII";
...
<%    if (obj.wSecurityAttribute > 0) { %>
            sec_addr[addrMgrConst::W_SEC_ADDR - 1] = m_axi_read_seq.m_ace_rd_addr_chnl_security;
<%    } %>
...
<%    if (obj.wSecurityAttribute > 0) { %>
            sec_addr[addrMgrConst::W_SEC_ADDR - 1] = m_axi_write_seq.m_ace_wr_addr_chnl_security;
<%    } %>
	    m_addr_mgr.set_addr_in_agent_mem_map(sec_addr, <%=obj.AiuInfo[obj.Id].FUnitId%>);
...
function bit [addrMgrConst::W_SEC_ADDR -1:0] get_addr_for_domain(bit [addrMgrConst::W_SEC_ADDR -1:0] start_addr,end_addr,int agentid,int core_id,bit is_rdnosnp);
  int primary_bits[$] = addrMgrConst::mp_aiu_intv_bits[agentid].pri_bits;
   bit [addrMgrConst::W_SEC_ADDR -1:0] new_addr,addr;
  ...
  do begin
   ...
   if(!is_rdnosnp) new_addr[5:0] = 0;
    ...
    check_addr_unconnected = addrMgrConst::check_unmapped_add(new_addr, agentid, unit_unconnect);
...
endclass : snps_axi_master_base_seq 
    ...
    function void create_len_for_connected_acess(bit [addrMgrConst::W_SEC_ADDR-1:0] addr,int size,output int incr_len,output int wrap_len);
    int native_core_id,core_id,temp_len;
    bit [addrMgrConst::W_SEC_ADDR-1:0] addr_n;
    bit [2:0] unit_unconnected;
    bit unconnected_access, native_unconnected_access,len_found;
    int timeout;
    
      
                addrMgrConst::extract_intlv_bits_in_addr(addrMgrConst::mp_aiu_intv_bits[<%=obj.FUnitId%>].pri_bits, addr, core_id);
    timeout =500;
   ...
   do begin
      ...
      unconnected_access = addrMgrConst::check_unmapped_add(addr_n,<%=obj.FUnitId%>,unit_unconnected);
      addrMgrConst::extract_intlv_bits_in_addr(addrMgrConst::mp_aiu_intv_bits[<%=obj.FUnitId%>].pri_bits, addr_n, core_id);
...
//                     m_rd_dvm.get(1);
	            ...
	            if(k_access_boot_region == 1) begin
	               use_addr_from_test = 1;
                       if(m_axi_read_seq.m_ace_rd_addr_chnl_snoop == svt_axi_transaction::READNOSNOOP) begin
                          m_ace_rd_addr_from_test = m_ace_cache_model.m_addr_mgr.get_noncohboot_addr(<%=obj.FUnitId%>, 1);
		       end else begin
                          m_ace_rd_addr_from_test = m_ace_cache_model.m_addr_mgr.get_cohboot_addr(<%=obj.FUnitId%>, 1);
                       end
                       <% if (obj.wSecurityAttribute > 0) { %>                                             
...
<% }} %>                                                
      ...
      else m_axi_read_seq.m_ace_rd_addr_chnl_addr = get_addr_for_domain(mst_cfg[core].innershareable_start_addr[0],mst_cfg[core].innershareable_end_addr[0],<%=obj.FUnitId%>,core,0);
...
<% }%> 
 	    ...
 	    //    			    ace_wr_addr_chnl_snoop = WRNOSNP;
		    ...
		    use_addr_from_test = 1;
		    if(m_axi_write_seq.m_ace_wr_addr_chnl_snoop == svt_axi_transaction::WRITENOSNOOP) begin
	               m_ace_wr_addr_from_test = m_ace_cache_model.m_addr_mgr.get_noncohboot_addr(<%=obj.FUnitId%>, 1);
                    end else begin
	               m_ace_wr_addr_from_test = m_ace_cache_model.m_addr_mgr.get_cohboot_addr(<%=obj.FUnitId%>, 1);
                    end
<% if (obj.wSecurityAttribute > 0) { %>                                             
...
<% }} %>                                                
      ...
      else m_axi_write_seq.m_ace_wr_addr_chnl_addr = get_addr_for_domain(mst_cfg[core].innershareable_start_addr[0],mst_cfg[core].innershareable_end_addr[0],<%=obj.FUnitId%>,core,0);
...
<% }} %>
   ...
   m_addr_mgr.get_dmi_unit_addr_range(start_addr0,end_addr0,start_addr1,end_addr1);
	...
	//read_tran.addr = get_addr_for_domain(read_tran.port_cfg.innershareable_start_addr[0],read_tran.port_cfg.innershareable_end_addr[0],<%=obj.FUnitId%>,core,0);
	read_tran.addr = get_addr_for_domain(read_tran.port_cfg.innershareable_start_addr[0],read_tran.port_cfg.innershareable_start_addr[0]+4000,<%=obj.FUnitId%>,core,0);
	...
	read_tran_1.addr = snoop_adr;
      ...
      if(read_tran_1.coherent_xact_type == svt_axi_transaction::READNOSNOOP) begin
          ...
          if(!size)begin
	  if(start_addr0 == read_tran_1.port_cfg.innershareable_start_addr[0]) read_tran_1.addr = get_addr_for_domain(start_addr1,end_addr1,<%=obj.FUnitId%>,core,0);
	  else read_tran_1.addr = get_addr_for_domain(start_addr0,end_addr0,<%=obj.FUnitId%>,core,0);
          end
      end else begin 
	  if(!size)read_tran_1.addr = get_addr_for_domain(read_tran_1.port_cfg.innershareable_start_addr[0],read_tran_1.port_cfg.innershareable_start_addr[0]+4000,<%=obj.FUnitId%>,core,0);
	...
	if(start_addr0 == write_tran.port_cfg.innershareable_start_addr[0]) write_tran.addr = get_addr_for_domain(start_addr1,end_addr1,<%=obj.FUnitId%>,core,0);
	else write_tran.addr = get_addr_for_domain(start_addr0,end_addr0,<%=obj.FUnitId%>,core,0);
      end else begin
	if(!size)write_tran.addr = get_addr_for_domain(write_tran.port_cfg.innershareable_start_addr[0],write_tran.port_cfg.innershareable_start_addr[0]+4000,<%=obj.FUnitId%>,core,0);
...
<% }} %>
   ...
   m_addr_mgr.get_dmi_unit_addr_range(start_addr0,end_addr0,start_addr1,end_addr1);
	...
	if(start_addr0 == read_tran.port_cfg.innershareable_start_addr[0]) read_tran.addr = get_addr_for_domain(start_addr1,end_addr1,<%=obj.FUnitId%>,core,0);
	else read_tran.addr = get_addr_for_domain(start_addr0,end_addr0,<%=obj.FUnitId%>,core,0);
    ...
    endcase
     ...
     //     
     // end else begin 
     //     read_tran_1.addr = get_addr_for_domain(read_tran_1.port_cfg.innershareable_start_addr[0],read_tran_1.port_cfg.innershareable_start_addr[0]+4000,<%=obj.FUnitId%>,core,0);
     //     read_tran_1.cache_type[1:0] = 2'b11;
     //     read_tran_1.domain_type = svt_axi_transaction::INNERSHAREABLE;
     // end
     // 
...
class snps_axi_master_outstanding_read_seq extends snps_axi_master_seq;
 ...
 int align;
  ...
  int master_id,core,incr_len,wrap_len;
   int primary_bits[$] = addrMgrConst::mp_aiu_intv_bits[<%=obj.FUnitId%>].pri_bits;
   bit check_addr_unconnected = 0;
   int timeout;
...
<% }} %>
	...
	//std::randomize(addr)with{addr inside {[start_addr_q[_tmp]:end_addr_q[_tmp]]};};
        addr  = get_addr_for_domain(start_addr_q[_tmp],end_addr_q[_tmp],<%=obj.FUnitId%>,core,1);
...
<% if ((obj.fnNativeInterface != "AXI4") ) { %>
  ...
  for(int i = 0; i <sequence_length; i++) begin
   ...
   foreach(load.port_cfg.innershareable_start_addr[i]) begin
        ...
        std::randomize(_tmp)with{_tmp inside {[0:(size-1)]};};
        coh_addr = get_addr_for_domain(start_addr_coh_q[_tmp],end_addr_coh_q[_tmp],<%=obj.FUnitId%>,core,0);
...
class snps_ace_master_read_seq extends snps_axi_master_seq;
  ...
  virtual task body();
	...
	if(read_tran.domain_type == svt_axi_transaction::SYSTEMSHAREABLE) begin
		//read_tran.addr = get_addr_for_domain(start_addr1,end_addr1,<%=obj.FUnitId%>,core);;
		if(start_addr0 == read_tran.port_cfg.outershareable_start_addr[0]) read_tran.addr = get_addr_for_domain(start_addr1,end_addr1,<%=obj.FUnitId%>,core,1);
		else read_tran.addr = get_addr_for_domain(start_addr0,end_addr0,<%=obj.FUnitId%>,core,1);
	end else if(read_tran.domain_type == svt_axi_transaction::OUTERSHAREABLE) begin
		read_tran.addr =  get_addr_for_domain(read_tran.port_cfg.outershareable_start_addr[0],read_tran.port_cfg.outershareable_end_addr[0],<%=obj.FUnitId%>,core,0);
	end else begin
		read_tran.addr =  get_addr_for_domain(read_tran.port_cfg.nonshareable_start_addr[0],read_tran.port_cfg.nonshareable_end_addr[0],<%=obj.FUnitId%>,core,0);
	end
...
class snps_ace_master_write_seq extends snps_axi_master_seq;
  ...
  virtual task body();
	...
	if(write_tran.domain_type == svt_axi_transaction::SYSTEMSHAREABLE) begin
		//write_tran.addr = get_addr_for_domain(start_addr1,end_addr1,<%=obj.FUnitId%>,core);;
		if(start_addr0 == write_tran.port_cfg.outershareable_start_addr[0]) write_tran.addr = get_addr_for_domain(start_addr1,end_addr1,<%=obj.FUnitId%>,core,1);
		else write_tran.addr = get_addr_for_domain(start_addr0,end_addr0,<%=obj.FUnitId%>,core,1);
	end else if(write_tran.domain_type == svt_axi_transaction::OUTERSHAREABLE) begin
		write_tran.addr =  get_addr_for_domain(write_tran.port_cfg.outershareable_start_addr[0],write_tran.port_cfg.outershareable_end_addr[0],<%=obj.FUnitId%>,core,0);
	end else begin
		write_tran.addr =  get_addr_for_domain(write_tran.port_cfg.nonshareable_start_addr[0],write_tran.port_cfg.nonshareable_end_addr[0],<%=obj.FUnitId%>,core,0);
	end
  ...
  virtual task body();
   ...
   bit [2:0] unit_unconnect = 0;
    ...
    if(AiuDii_connectivity_vec != exp_aiudii_connectivity_vec)begin
      ...
      foreach(dii_connect[j])begin
         ...
         foreach (csrq[i]) begin
           if(csrq[i].unit == addrMgrConst::DII && csrq[i].mig_nunitid == dii_id) begin
    ...
    for(int i = 0; i <sequence_length; i++) begin
      ...
      if(dii_access)begin
        std::randomize(_tmp)with{_tmp inside {[0:(size-1)]};};
        rdnsnp_addr = get_addr_for_domain(start_addr_coh_q[_tmp],end_addr_coh_q[_tmp],<%=obj.FUnitId%>,core,1);
      end else begin
        check_addr_unconnected = addrMgrConst::check_unmapped_add(read_tran.port_cfg.nonshareable_start_addr[0], <%=obj.FUnitId%>, unit_unconnect);
        if(check_addr_unconnected==0)rdnsnp_addr = get_addr_for_domain(read_tran.port_cfg.nonshareable_start_addr[0],read_tran.port_cfg.nonshareable_end_addr[0],<%=obj.FUnitId%>,core,1);
      end
 
     check_addr_unconnected = addrMgrConst::check_unmapped_add(rdnsnp_addr, <%=obj.FUnitId%>, unit_unconnect);
	...
	tmp_len=1;
      ...
      constrain_4k=0;
      if(dii_access)rdnsnp_addr = get_addr_for_domain(start_addr_coh_q[_tmp],end_addr_coh_q[_tmp],<%=obj.FUnitId%>,core,1);
      else rdnsnp_addr = get_addr_for_domain(write_tran.port_cfg.nonshareable_start_addr[0],write_tran.port_cfg.nonshareable_end_addr[0],<%=obj.FUnitId%>,core,1);
      create_len_for_connected_acess(rdnsnp_addr,<%=Math.log2(aiu_axiInt.params.wData/8)%>,incr_len,wrap_len);
      constrain_len=1;
      if((rdnsnp_addr[11:0] + ((<%=obj.AiuInfo[obj.Id].wData/8%>)*1)) >= 4096 )begin
        rdnsnp_addr[11:0]= rdnsnp_addr[11:0] -((<%=obj.AiuInfo[obj.Id].wData/8%>)*1);
