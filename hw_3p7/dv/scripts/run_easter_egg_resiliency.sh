...
echo "which vsim" >> $date_dir/gen_config.sh
echo "echo \$LM_LICENSE_FILE" >> $date_dir/gen_config.sh
echo "node $WORK_TOP/dv/scripts/gen_rand_configs.js -D $date_dir -n $num_configs -f $WORK_TOP/dv/scripts/configHelperResiliency.json" >> $date_dir/gen_config.sh
...
for j in `seq 0 $last_config`; do
    echo "node $WORK_TOP/dv/scripts/rsim.js -e psys -a -r -c -m -q -n rand$j -d INHOUSE_ACE_BFM,V1,SV_PSEUDO_NOC,PSEUDO_SYS_TB,DUMP_ON,DISABLE_STRRSP_TR_CHECK,EXCLUSIVE_MON,SFI_DELAY_DISABLE,RESILIENCY_TESTING -s $date_dir/config``$j``.json -l -L" >> $date_dir/run_config_list
...
#then
#    echo "ERROR! Something failed with generating configs!"
#    exit 1;
#else
