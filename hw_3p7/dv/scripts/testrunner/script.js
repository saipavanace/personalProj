        ...
        rows:[
            {view:"toolbar", cols:[
              //  { view:"search" },
                ...
                { view:"counter", id:"counter_id", label:"Parallel Jobs", labelPosition:"top", width:100, value:10},
                { view:"button", value:"Go!", inputWidth:80, type:"danger", click:"sel()"}
//                { view:"button", value:"Kill All!", inputWidth:80, type:"danger", click:"kill()"}
...
//    })
    ...
    var addItems= function(data, number, name, cwd){
        ...
        for(var i = 0; i<lines.length; i++) {
            var line = lines[i];
//            console.log(line);
            line = line.replace(/\$WORK_TOP/g, window.hwpath);
//            console.log(line);
...
function sel(){
    ...
    // TODO:  This runs forever, which seems bad.
    interval = setInterval(function(){
        ...
        tree.data.each(function(row){
            ...
            allowedthreads = $$("counter_id").getValue();

            if (threadsused < allowedthreads) {
                if (tree.isChecked(id)) {
                    var d = tree.getItem(id);
                    if(d.status=="+Waiting for thread.") {
...
//                        console.log("Start ("+threadsused+"/"+allowedthreads+"):"+args);
                        d.stdout = "";
