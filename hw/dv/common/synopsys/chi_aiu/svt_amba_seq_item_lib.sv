...
<% } %>
  ...
  static ncoreConfigInfo::addrq user_addrq[];
  static int 		      user_addrq_idx[];
  static ncoreConfigInfo::addrq user_write_addrq[];
  static int 		      user_write_addrq_idx[];
  static ncoreConfigInfo::addrq user_read_addrq[];
...
<% } %>
   ...
   bit [ncoreConfigInfo::W_SEC_ADDR -1:0] pick_soft_addr;
...
<% } %>
   ...
   constraint svt_chi_item_c_addr {
      if((unmapped_addr_access_test == 0 && svt_chi_item_helper::dis_addr_range_constraint==0 && user_addr==0 <% if (obj.testBench != "fsys") { %> && (add_selfid_rd == 1) && (txn_counter > 0)<% } %> ) || (unsupported_atomic_txn_to_dii == 1) <% if (obj.testBench != "fsys") { %>||(add_selfid_rd == 0 && unmapped_addr_access_test == 0)<% } %>) {
          if(ncoreConfigInfo::dmi_memory_domain_start_addr.size() > 0) {
            dmi_memory_domain_index < ncoreConfigInfo::dmi_memory_domain_start_addr.size();
          }
          if(ncoreConfigInfo::dii_memory_domain_start_addr.size() > 0) {
            dii_memory_domain_index < ncoreConfigInfo::dii_memory_domain_start_addr.size();
          }
         ...
         // (m_addr_type==<%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::NON_COH_ADDR) -> ncore_unit_type inside {DMI,DII};
          //(ncoreConfigInfo::dmi_memory_domain_start_addr.size() > 0) -> ncore_unit_type == DMI;
          //(ncoreConfigInfo::dii_memory_domain_start_addr.size() > 0) -> ncore_unit_type == DII;
          if (m_addr_type==<%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::NON_COH_ADDR) {
                 if ((ncoreConfigInfo::dmi_memory_domain_start_addr.size() > 0) && (ncoreConfigInfo::dii_memory_domain_start_addr.size() > 0)) {
                      ...
                      if(unsupported_atomic_txn_to_dii == 0) {
                        if ((ncoreConfigInfo::dmi_memory_domain_start_addr.size() > 0) && (ncoreConfigInfo::dii_memory_domain_start_addr.size() == 0)) {
                           ...
                           ((ncoreConfigInfo::dii_memory_domain_start_addr.size() > 0) && (ncoreConfigInfo::dmi_memory_domain_start_addr.size() == 0)) -> ncore_unit_type == DII;  
                        }
                      } else {
			ncore_unit_type == DII;
                      }
          ...
          if(m_addr_type==<%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::COH_ADDR) {
                 if(overlapping){
                 foreach (ncoreConfigInfo::dmi_memory_domain_start_addr[i]) {
                   ((dmi_memory_domain_index == i) && (ncore_unit_type==DMI) && (cfg.enable_domain_based_addr_gen == 0) && (m_opcode_type != <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::DVM_OPERT_CMD)) ->  addr inside {[ncoreConfigInfo::dmi_memory_domain_start_addr[i]:(ncoreConfigInfo::dmi_memory_domain_start_addr[i]+20)]};
                 }
                }
                else{
                  foreach (ncoreConfigInfo::dmi_memory_domain_start_addr[i]) {
                   ((dmi_memory_domain_index == i) && (ncore_unit_type==DMI) && (cfg.enable_domain_based_addr_gen == 0) && (m_opcode_type != <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::DVM_OPERT_CMD)) ->  addr inside {[ncoreConfigInfo::dmi_memory_domain_start_addr[i]:(ncoreConfigInfo::dmi_memory_domain_end_addr[i])]};
                 }
                }                 
          } else {
           if(unsupported_atomic_txn_to_dii == 0) {
                  if(overlapping){
                 foreach (ncoreConfigInfo::dmi_memory_domain_start_addr[i]) {
                   ((dmi_memory_domain_index == i) && (ncore_unit_type==DMI) && (cfg.enable_domain_based_addr_gen == 0) && (m_opcode_type != <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::DVM_OPERT_CMD)) ->  addr inside {[ncoreConfigInfo::dmi_memory_domain_start_addr[i]:(ncoreConfigInfo::dmi_memory_domain_start_addr[i]+10)]};
                 }
                }
                else{
                  foreach (ncoreConfigInfo::dmi_memory_domain_start_addr[i]) {
                 ...
                 foreach (ncoreConfigInfo::dii_memory_domain_start_addr[i]) {
                   ((dii_memory_domain_index == i) && (ncore_unit_type==DII) && (cfg.enable_domain_based_addr_gen == 0)) -> addr inside {[ncoreConfigInfo::dii_memory_domain_start_addr[i]:
                  ...
                  if((ncoreConfigInfo::dii_memory_domain_start_addr.size() > 0)){
                      if(connectivity_test == 0){
                          foreach (ncoreConfigInfo::dii_memory_domain_start_addr[i]) {
                   	    ((dii_memory_domain_index == i) && (cfg.enable_domain_based_addr_gen == 0)) -> addr inside {[ncoreConfigInfo::dii_memory_domain_start_addr[i]:
                                                                       ncoreConfigInfo::dii_memory_domain_end_addr[i]], [ncoreConfigInfo::NRS_REGION_BASE:ncoreConfigInfo::NRS_REGION_BASE + ncoreConfigInfo::NRS_REGION_SIZE-1]}; 
                      }
		      } else {
                          foreach (ncoreConfigInfo::dii_memory_domain_start_addr[i]) {
                   	    ((dii_memory_domain_index == i) && (cfg.enable_domain_based_addr_gen == 0)) -> addr inside {[ncoreConfigInfo::dii_memory_domain_start_addr[i]:
...
// `ifndef SVT_CHI_ISSUE_A_ENABLE
      constraint svt_chi_opcode_type_wise_c {
               ...
               (m_opcode_type == <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::DT_LS_UPD_CMD) || (m_opcode_type == <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::DT_LS_CMO_CMD) || (m_opcode_type == <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::DT_LS_STH_CMD)
                 ...
                 (stash_nid_valid == 1) -> stash_nid inside {ncoreConfigInfo::aiu_nids};
               }
      }
// `endif // SVT_CHI_ISSUE_A_ENABLE
`ifdef SVT_CHI_ISSUE_E_ENABLE
    ...
    //  `endif
     ...
     //       }
      ...
      constraint c_mem_type {
            if(ncoreConfigInfo::dii_memory_domain_start_addr.size() > 0 || (m_boot_addr) || ((add_selfid_rd == 1'h1) && (txn_counter == 0))) {  
...
if(align_unalign_addr) begin
  ...
  if(unmapped_addr_access_test != 0 && ncoreConfigInfo::map_addr2dmi_or_dii(addr, fnmem_region_idx) != -1) begin
    if(ncoreConfigInfo::is_dii_addr(addr) == 1) begin
      snp_attr_is_snoopable = 0;
      m_addr_type =  <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::NON_COH_ADDR;
      ncore_unit_type = DII;
...
<% } %>
 ...
 if(same_addr_test || user_addr_for_csr || user_addr || pick_boundary_addr_snps)begin 
  ...
  if(atomic_cfg_45)begin
    ...
    `uvm_info(get_name(),$psprintf("svt_chi_item::post_randomize addr 'h%0h dmi_memory_domain_index %0d dii_memory_domain_index %0d ncore_unit_type %0s xact_type %0s start_ix %0d, force_cleanuniq %0d, m_tgtid %0d, m_opcode_type %0s, m_opcode %0s, m_addr_type %0s, m_new_addr %0d, m_boot_addr %0d,m_excl_txn %0d, m_excl_noncoh_txn %0d, m_qos %0d, m_mem_type %0s, m_ewa %0d, m_snpattr %0d, m_snoopme %0d, m_lpid %0d data_size %0s addr_zero_bits %0d qos %0d endian %0d stash nid valid %0d",addr,dmi_memory_domain_index,dii_memory_domain_index,ncore_unit_type.name,xact_type.name,start_ix, force_cleanuniq, m_tgtid, m_opcode_type, m_opcode, m_addr_type, m_new_addr, m_boot_addr,m_excl_txn, m_excl_noncoh_txn, m_qos, m_mem_type, m_ewa, m_snpattr, m_snoopme, m_lpid,data_size.name,addr_zero_bits,qos,endian, stash_nid_valid),UVM_NONE)
     //foreach(ncoreConfigInfo::dmi_memory_domain_start_addr[i])
     //  `uvm_info(get_name(),$psprintf("%0d dmi_memory_domain_start_addr %0h dmi_memory_domain_end_addr %0h",i,ncoreConfigInfo::dmi_memory_domain_start_addr[i],ncoreConfigInfo::dmi_memory_domain_end_addr[i]),UVM_DEBUG)
     //foreach(ncoreConfigInfo::dmi_memory_domain_start_addr[i])
     //  `uvm_info(get_name(),$psprintf("%0d dii_memory_domain_start_addr %0h dii_memory_domain_end_addr %0h",i,ncoreConfigInfo::dii_memory_domain_start_addr[i],ncoreConfigInfo::dii_memory_domain_end_addr[i]),UVM_DEBUG)
 ...
 if (m_opcode_type inside {<%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::ATOMIC_LD_CMD, <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::ATOMIC_ST_CMD, <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::ATOMIC_CM_CMD, <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::ATOMIC_SW_CMD} &&
  ...
  if (svt_chi_rn_transaction::mem_attr_mem_type == DEVICE && (m_opcode_type != <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::DVM_OPERT_CMD) && (m_opcode_type != <%=chi_bfm_types_pkg_prefix%>_chi_bfm_types_pkg::RQ_LCRDRT_CMD))begin
   ...
   function bit[`SVT_CHI_MAX_ADDR_WIDTH-1:0] svt_chi_item::pick_atomic_addr();
     bit [ncoreConfigInfo::W_SEC_ADDR -1:0] t_addr;
 ...
 function bit[`SVT_CHI_MAX_ADDR_WIDTH-1:0] svt_chi_item::pick_nrs_region_addr();
   ...
   int boot_addr_offset;
    ...
    t_addr = ncore_config_pkg::ncoreConfigInfo::NRS_REGION_BASE ; //  addr = {'h00b9b90, 8'hFF, 12'h000};
    if($test$plusargs("boot_addr_offset")) begin
        t_addr = ncore_config_pkg::ncoreConfigInfo::BOOT_REGION_BASE ; //  addr = {'h00b9b90, 8'hFF, 12'h000};
    end
 ...
 function bit[`SVT_CHI_MAX_ADDR_WIDTH-1:0] svt_chi_item::get_normal_noncoh_mem_addr(bit req_new_addr);
   ...
   //`uvm_info($sformatf("%m"), $sformatf("svt_chi_item-DEBUG,get_normal_noncoh_mem_addr: use_user_addrq %0d size %0d",$test$plusargs("use_user_addrq"), user_addrq[ncoreConfigInfo::NONCOH].size()), UVM_MEDIUM)
        if($test$plusargs("user_addr_for_csr")) begin
          user_addrq[ncoreConfigInfo::NONCOH] = ncoreConfigInfo::user_addrq[ncoreConfigInfo::NONCOH];
        end
   ...
   if($test$plusargs("use_user_addrq") && (user_addrq[ncoreConfigInfo::NONCOH].size()>0)) begin
      if(user_addrq_idx[ncoreConfigInfo::NONCOH] == -1) begin
         ...
         t_addr = user_addrq[ncoreConfigInfo::NONCOH][pick_itr];
      // `uvm_info($sformatf("%m"), $sformatf("svt_chi_item-DEBUG,get_normal_noncoh_mem_addr: generated user NC-1 address %p", t_addr), UVM_MEDIUM)
         ...
         t_addr = user_addrq[ncoreConfigInfo::NONCOH][user_addrq_idx[ncoreConfigInfo::NONCOH]];
      ...
      if (req_new_addr) begin
         ...
         user_addrq[ncoreConfigInfo::NONCOH].push_back(t_addr);
...
// `uvm_info($sformatf("%m"), $sformatf("svt_chi_item-DEBUG,get_normal_coh_mem_addr: use_user_addrq %0d size %0d",$test$plusargs("use_user_addrq"), user_addrq[ncoreConfigInfo::COH].size()), UVM_MEDIUM)

        if($test$plusargs("user_addr_for_csr")) begin
          user_addrq[ncoreConfigInfo::COH] = ncoreConfigInfo::user_addrq[ncoreConfigInfo::COH];
        end
   ...
   if($test$plusargs("use_user_addrq") && (user_addrq[ncoreConfigInfo::COH].size()>0)) begin
      if(user_addrq_idx[ncoreConfigInfo::COH] == -1) begin
         ...
         t_addr = user_addrq[ncoreConfigInfo::COH][pick_itr];
      // `uvm_info($sformatf("%m"), $sformatf("svt_chi_item-DEBUG,get_normal_coh_mem_addr : generated user C-1 address %p", t_addr), UVM_MEDIUM)
      end
      else begin
         t_addr = user_addrq[ncoreConfigInfo::COH][user_addrq_idx[ncoreConfigInfo::COH]];
      ...
      if (req_new_addr) begin
        ...
        user_addrq[ncoreConfigInfo::COH].push_back(t_addr);
