...
class cust_svt_amba_system_configuration extends svt_amba_system_configuration;
  ...
  ncoreConfigInfo::sys_addr_csr_t slave_cfg_csrq[$];
  ncoreConfigInfo::ncore_unit_type_t slave_cfg_nintrlv_type[$];
...
function void cust_svt_amba_system_configuration::set_chi_system_configuration(svt_chi_system_configuration chi_sys_cfg);
  ...
  bit [63:0] start_addr;
  bit [63:0] end_addr;
  bit [ncoreConfigInfo::W_SEC_ADDR - 1 : 0] k_unmapped_add_access_wgt_snps;
    ...
    bit dont_divide_nonsnoopable_region_among_CHIs;
        ...
        foreach (ncoreConfigInfo::memregions_info[region]) begin
            if (!(ncoreConfigInfo::is_dii_addr(ncoreConfigInfo::memregions_info[region].start_addr) ||
                (ncoreConfigInfo::is_dmi_addr(ncoreConfigInfo::memregions_info[region].start_addr) && ncoreConfigInfo::get_addr_gprar_nc(ncoreConfigInfo::memregions_info[region].start_addr)))) begin
                
                slave_id = ncoreConfigInfo::map_addr2dmi_or_dii(ncoreConfigInfo::memregions_info[region].start_addr,fnmem_region_idx);
        ...
        <%if (obj.testBench == "fsys") { %>
            if(k_chiaiu_access_boot_region) begin : _k_chiaiu_access_boot_region
                    ...
                    if ($test$plusargs("reduce_addr_area")) // reduce number of addr to allow more snoop
                        chi_sys_cfg.set_addr_for_domain(0, ncoreConfigInfo::BOOT_REGION_BASE, (ncoreConfigInfo::BOOT_REGION_BASE + (reduce_mem_size<< 6)));
                    else
                        chi_sys_cfg.set_addr_for_domain(0, ncoreConfigInfo::BOOT_REGION_BASE, (ncoreConfigInfo::BOOT_REGION_BASE + ncoreConfigInfo::BOOT_REGION_SIZE-1));
            ...
            foreach (ncoreConfigInfo::memregions_info[region]) begin
                if (ncoreConfigInfo::is_dii_addr(ncoreConfigInfo::memregions_info[region].start_addr) ||
                    ...
                    s = ncoreConfigInfo::memregions_info[region].start_addr;
                    if ($test$plusargs("reduce_addr_area")) begin // reduce number of addr to allow more snoop
                    e = ncoreConfigInfo::memregions_info[region].start_addr + (reduce_mem_size<< <%=obj.wCacheLineOffset%>) ;
                    end else begin
                    e = ncoreConfigInfo::memregions_info[region].end_addr;
                    end
                    ...
                    d = (e - s)/((dont_divide_nonsnoopable_region_among_CHIs==0)?<%=obj.nCHIs%>:1);
                    ...
                    if ($test$plusargs("reduce_addr_area")) begin // reduce number of addr to allow more snoop
                      chi_sys_cfg.set_addr_for_domain(domain_count, ncoreConfigInfo::memregions_info[region].start_addr, ncoreConfigInfo::memregions_info[region].start_addr+(reduce_mem_size<< <%=obj.wCacheLineOffset%>));
                      if (ncoreConfigInfo::memregions_info[region].start_addr+(reduce_mem_size<< <%=obj.wCacheLineOffset%>) < ncoreConfigInfo::memregions_info[region].end_addr-(reduce_mem_size<< <%=obj.wCacheLineOffset%>)) begin:_avoid_overlap
                         chi_sys_cfg.set_addr_for_domain(domain_count, ncoreConfigInfo::memregions_info[region].end_addr-(reduce_mem_size<< <%=obj.wCacheLineOffset%>), ncoreConfigInfo::memregions_info[region].end_addr);
                      end:_avoid_overlap
                    end else begin 
                    chi_sys_cfg.set_addr_for_domain(domain_count, ncoreConfigInfo::memregions_info[region].start_addr, ncoreConfigInfo::memregions_info[region].end_addr);
                    end
                    `uvm_info(get_name(),$psprintf("cust_svt_amba_system_configuration:: Setting address for DMI domain (id=%0d)", domain_count),UVM_NONE)
                    if ($test$plusargs("unmapped_addr_access")) begin // create dummy space address to check unmapped addr
                       chi_sys_cfg.set_addr_for_domain(domain_count,ncoreConfigInfo::memregions_info[region].end_addr,ncoreConfigInfo::memregions_info[region].end_addr+1000);
            ...
            begin:_nrs_addr_area
               ...
               chi_sys_cfg.set_addr_for_domain(chi_csr_nbr, ncoreConfigInfo::NRS_REGION_BASE, ncoreConfigInfo::NRS_REGION_BASE + ncoreConfigInfo::NRS_REGION_SIZE-1);
            end:_nrs_addr_area
        ...
        <%if (obj.testBench == "chi_aiu") { %>
            foreach (csrq[i]) begin
                if(csrq[i].unit == ncoreConfigInfo::DII && chi_aiu_non_snoopable_domain_created==0) begin
                ...
                if(csrq[i].unit == ncoreConfigInfo::DMI && chi_aiu_inner_snoopable_domain_created==0) begin
                ...
                if(csrq[i].unit == ncoreConfigInfo::DMI) begin
                    //domain_size = (1 << (csrq[i].size+12)+$clog2(ncoreConfigInfo::nmig));
	    ...
	    <% if (obj.testBench == "fsys") {%>
            ...
            `ifdef SVT_CHI_ISSUE_B_ENABLE
                if(chi_sys_cfg.rn_cfg[<%=chiaiu_idx%>].chi_spec_revision == svt_chi_node_configuration::ISSUE_B) begin
                    ...
                    foreach(ncoreConfigInfo::stash_nids[stash_nids_len]) begin
                        if(ncoreConfigInfo::stash_nids[stash_nids_len]!=<%=obj.ChiaiuInfo[chiaiu_idx].nUnitId%>) begin
                            ...
                            chi_sys_cfg.rn_cfg[<%=chiaiu_idx%>].valid_stash_tgt_id[chi_sys_cfg.rn_cfg[<%=chiaiu_idx%>].valid_stash_tgt_id.size()-1] = ncoreConfigInfo::stash_nids[stash_nids_len];
                        end
                    end
                end
            `endif
...
function void cust_svt_amba_system_configuration::set_axi_system_configuration(svt_axi_system_configuration axi_sys_cfg);
  ...
  axi_sys_cfg.wdata_watchdog_timeout                                                                                           ='d1024000;
  axi_sys_cfg.awaddr_watchdog_timeout                                                                                          ='d1024000;
  void'($value$plusargs("axi_sys_enable_domain_based_addr_gen=%0b",axi_sys_enable_domain_based_addr_gen));
  if ($test$plusargs("disable_bresp_watchdog_timeout")) begin
      axi_sys_cfg.bresp_watchdog_timeout                                                                                       ='d0;
  end else begin
      axi_sys_cfg.bresp_watchdog_timeout                                                                                       ='d1024000;
  end
  ...
  coh_regionsq = m_map.get_coh_mem_regions();
     ...
     /*   foreach (csrq[i]) begin
        ...
        foreach (csrq[i]) begin
          if(csrq[i].unit == ncoreConfigInfo::DII && ace_non_snoopable_domain_created==0) begin
            non_snoopable_domain_id = domain_type_cnt;
          ...
          if(csrq[i].unit == ncoreConfigInfo::DMI && ace_inner_snoopable_domain_created==0) begin
            inner_snoopable_domain_id = domain_type_cnt;
          ...
          if(csrq[i].unit == ncoreConfigInfo::DMI) begin
            ...
            ncoreConfigInfo::dmi_memory_domain_start_addr.push_back(start_addr);
            ncoreConfigInfo::dmi_memory_domain_end_addr.push_back(end_addr);
            `uvm_info(get_name(),$psprintf("cust_svt_amba_system_configuration:: set_addr_for_domain INNERSNOOPABLE  domain_id %0d start_addr 0x%h end_addr 0x%h domain_size 'd%0d nmig %0d",inner_snoopable_domain_id,start_addr,end_addr,domain_size,ncoreConfigInfo::nmig),UVM_NONE)
          end
        end
*/
//=========BASIC_AXI_CFG===================================
    ...
    axi_sys_cfg.rready_watchdog_timeout = `SVT_AXI_MAX_AXI3_GENERIC_DELAY + 2000;
    axi_sys_cfg.bready_watchdog_timeout = `SVT_AXI_MAX_AXI3_GENERIC_DELAY + 2000;
//=========ADDRESS_SPACE==============================
    //stimulus

`ifdef USE_VIP_SNPS_AXI_SLAVES
    //reg booting seq
    //axi_sys_cfg.set_addr_range(-1, ncoreConfigInfo::NRS_REGION_BASE, (ncoreConfigInfo::NRS_REGION_BASE + ncoreConfigInfo::NRS_REGION_SIZE-1) );    //<------CSR (DII SYS)
    //axi_sys_cfg.set_addr_range(-1, ncoreConfigInfo::BOOT_REGION_BASE, (ncoreConfigInfo::BOOT_REGION_BASE + ncoreConfigInfo::BOOT_REGION_SIZE-1) ); //<------BOOT
  foreach (m_mem.nintrlv_grps[i]) begin
     slave_cfg_csrq = ncoreConfigInfo::get_memregions_assoc_ig(i);
	...
	if(slave_cfg_csrq[j].unit == ncoreConfigInfo::DMI) begin
           for(int unit=0; unit<m_mem.nintrlv_grps[i]; unit=unit+1) begin
              ...
              int nDmis_per_ig = ncoreConfigInfo::intrlvgrp_vector[ncoreConfigInfo::picked_dmi_igs][ig];
                ...
                if(m_mem.nintrlv_grps[i]>1 && unit>0) begin
                    ...
                    port_interleaving_size.push_back(1<<(ncoreConfigInfo::dmi_sel_bits[ncoreConfigInfo::picked_dmi_if[nDmis_per_ig]][nDmis_per_ig].pri_bits[ncoreConfigInfo::dmi_sel_bits[ncoreConfigInfo::picked_dmi_if[nDmis_per_ig]][nDmis_per_ig].pri_bits.size() -1]));
	...
	end // if (slave_cfg_csrq[j].unit == ncoreConfigInfo::DMI)
	else begin
           //port_interleaving_enable.push_back(0);
           ...
           slave_cnt = slave_cnt + 1;
        end // else: !if(slave_cfg_csrq[j].unit == ncoreConfigInfo::DMI)
  ...
  axi_sys_cfg.set_addr_range(<%=obj.AiuInfo[0].BootInfo.regionHui%>, ncoreConfigInfo::BOOT_REGION_BASE, (ncoreConfigInfo::BOOT_REGION_BASE + ncoreConfigInfo::BOOT_REGION_SIZE-1) ); //<------BOOT
  <% } else { %>
  //$sformat(s, "%s %s %0d           |", s, "DII", <%=obj.AiuInfo[0].BootInfo.regionHui%>);
  axi_sys_cfg.set_addr_range(<%=obj.AiuInfo[0].BootInfo.regionHui +  obj.nDMIs%>, ncoreConfigInfo::BOOT_REGION_BASE, (ncoreConfigInfo::BOOT_REGION_BASE + ncoreConfigInfo::BOOT_REGION_SIZE-1) ); //<------BOOT
  <% } %>
...
<%}%>
    ...
    axi_sys_cfg.master_cfg[<%=axiaiu_idx%>].transaction_coverage_enable  = axi_snps_master_fcov_en;
...
<%axiaiu_idx++;}}}}%>
   ...
   `ifdef IO_UNITS_CNT_NON_ZERO 
    foreach(ncoreConfigInfo::io_subsys_nativeif_a[i]) begin
      ...
      axi_sys_cfg.master_cfg[i].cov_bins_dvm_tlbi_num_outstanding_xacts  = ncoreConfigInfo::num_outstanding_xacts[i];
      axi_sys_cfg.master_cfg[i].tagged_address_space_attributes_enable = 1;
      if (ncoreConfigInfo::io_subsys_nativeif_a[i] == "ACELITE-E") begin 
        axi_sys_cfg.master_cfg[i].atomic_transactions_enable = ncoreConfigInfo::io_subsys_atomic_enable_a[i];
        axi_sys_cfg.master_cfg[i].use_separate_rd_wr_chan_id_width = 0;
        axi_sys_cfg.master_cfg[i].id_width = ncoreConfigInfo::AXID_WIDTH;
      end
    ...
    axi_sys_cfg.master_cfg[i].state_coverage_enable            = 1;
    axi_sys_cfg.master_cfg[i].transaction_coverage_enable      = 1;
    end
    `endif
...
//`ifdef IO_SUBSYS_SNPS
      if(axi_sys_enable_domain_based_addr_gen) begin
            ...
            foreach (csrq[i]) begin
                if((csrq[i].unit == ncoreConfigInfo::DII) || (csrq[i].unit == ncoreConfigInfo::DMI && csrq[i].nc==1)) begin
                    start_addr = {csrq[i].upp_addr, csrq[i].low_addr} << 12;
                    foreach(io_subsys_nonshareable_domain_created[x]) begin
                        ...
                        if(csrq[i].unit == ncoreConfigInfo::DMI && csrq[i].nc==1)
                            domain_size = (1 << (csrq[i].size+12)+$clog2(ncoreConfigInfo::intrlvgrp_vector[ncoreConfigInfo::picked_dmi_igs][csrq[i].mig_nunitid]));
                        else
                            domain_size = (1 << (csrq[i].size+12));
                        ...
                        `uvm_info(get_name(),$psprintf("cust_svt_amba_system_configuration:: domain_id %0d NONSHAREABLE(%0s) start_addr :0x%0h end_addr:0x%0h for masters:{%0p}",io_subsys_nonshareable_domain_id,(csrq[i].unit == ncoreConfigInfo::DMI)?"DMI":"DII",start_addr,end_addr, io_subsys_innershareable_master_ports_array[x]),UVM_LOW)
                    ...
                    start_addr = {csrq[i].upp_addr, csrq[i].low_addr} << 12;
                       if(!set_domain )begin 
                          ...
                          domain_size = (1 << (csrq[i].size+12)+$clog2(ncoreConfigInfo::intrlvgrp_vector[ncoreConfigInfo::picked_dmi_igs][csrq[i].mig_nunitid]));
                          end_addr = start_addr + domain_size - 1; 
                          `uvm_info(get_name(),$psprintf("cust_svt_amba_system_configuration:: domain_id %0d INNERSHAREABLE(%0s) start_addr 'h%h end_addr 'h%h for masters:{%0p}",io_subsys_innershareable_domain_id,(csrq[i].unit == ncoreConfigInfo::DMI)?"DMI":"DII",start_addr,end_addr, io_subsys_innershareable_master_ports_array),UVM_LOW)
                          axi_sys_cfg.set_addr_for_domain(io_subsys_innershareable_domain_id, start_addr, end_addr); 
                            ...
                            domain_size = (1 << (csrq[i].size+12)+$clog2(ncoreConfigInfo::intrlvgrp_vector[ncoreConfigInfo::picked_dmi_igs][csrq[i].mig_nunitid]));
                            end_addr = start_addr + domain_size - 1;
                            `uvm_info(get_name(),$psprintf("cust_svt_amba_system_configuration:: domain_id %0d OUTERSHAREABLE(%0s) start_addr 'h%h end_addr 'h%h for masters:{%0p}",io_subsys_outershareable_domain_id,(csrq[i].unit == ncoreConfigInfo::DMI)?"DMI":"DII",start_addr,end_addr, io_subsys_outershareable_master_ports_array),UVM_LOW)
...
//                end
//                else if(csrq[i].unit == ncoreConfigInfo::DMI) begin
//                    domain_size = (1 << (csrq[i].size+12)+$clog2(ncoreConfigInfo::intrlvgrp_vector[ncoreConfigInfo::picked_dmi_igs][csrq[i].mig_nunitid]));
//                    end_addr = start_addr + domain_size - 1;
//                    ncoreConfigInfo::dmi_memory_domain_start_addr.push_back(start_addr);
//                    ncoreConfigInfo::dmi_memory_domain_end_addr.push_back(end_addr);
...
//                end
            ...
            foreach (csrq[i]) begin
                ...
                if(csrq[i].unit == ncoreConfigInfo::DII) begin
                    ...
                    svt_axi_item_helper::all_dmi_dii_addr_range_start_addr.push_back(start_addr);
                    svt_axi_item_helper::all_dmi_dii_addr_range_end_addr.push_back(end_addr);
                    `uvm_info(get_name(),$psprintf("cust_svt_amba_system_configuration:: dii[%0d] start_addr 'h%h end_addr 'h%h",ncoreConfigInfo::dii_memory_domain_start_addr.size()-1,start_addr,end_addr),UVM_NONE)
                end
                if(csrq[i].unit == ncoreConfigInfo::DMI) begin
                    //domain_size = (1 << (csrq[i].size+12)+$clog2(ncoreConfigInfo::nmig));
                    domain_size = (1 << (csrq[i].size+12)+$clog2(ncoreConfigInfo::intrlvgrp_vector[ncoreConfigInfo::picked_dmi_igs][csrq[i].mig_nunitid]));
                    ...
                    svt_axi_item_helper::all_dmi_dii_addr_range_start_addr.push_back(start_addr);
                    svt_axi_item_helper::all_dmi_dii_addr_range_end_addr.push_back(end_addr);
                    `uvm_info(get_name(),$psprintf("cust_svt_amba_system_configuration:: dmi[%0d] start_addr 'h%h end_addr 'h%h",ncoreConfigInfo::dmi_memory_domain_start_addr.size()-1,start_addr,end_addr),UVM_NONE)
                end
            end
    end
   ...
   if(axi_sys_cfg.master_cfg.size()>0) begin
     foreach(axi_sys_cfg.master_cfg[i]) begin
         axi_sys_cfg.master_cfg[i].set_port_name(ncoreConfigInfo::io_subsys_instname_a[i]);
         `uvm_info(get_name(),$psprintf("POST_DOMAIN_BASED_ADDR_GEN::cust_svt_amba_system_configuration:: axi_sys_cfg.master_cfg[%0d] with port_name:%0s",i, ncoreConfigInfo::io_subsys_instname_a[i]),UVM_NONE)
         axi_sys_cfg.master_cfg[i].print();
     end
   end
 `endif
...
<% if (obj.AiuInfo[0].fnNativeInterface == 'CHI-A') { %>
        foreach (csrq[i]) begin
          ...
          if(csrq[i].unit == ncoreConfigInfo::DII && onetime_dii_creation == 0) begin //{
           ...
           for(int k=0; k< <%=chiaiuIndexNum%>; k++) begin
            ...
            if(set_nonsnp[<%=rn%>] == 0) begin
              ...
              ncoreConfigInfo::dii_memory_domain_start_addr.push_back(rn_dii_start_addr[<%=rn%>]);
              ncoreConfigInfo::dii_memory_domain_end_addr.push_back(rn_dii_end_addr[<%=rn%>]);
          ...
          if(csrq[i].unit == ncoreConfigInfo::DMI && onetime_dmi_creation == 0) begin //{
           ...
           for(int k=0; k< <%=chiaiuIndexNum%>; k++) begin
            ...
            if(set_innsnp[<%=rn%>] == 0) begin
              ...
              ncoreConfigInfo::dmi_memory_domain_start_addr.push_back(rn_dmi_start_addr[<%=rn%>]);
              ncoreConfigInfo::dmi_memory_domain_end_addr.push_back(rn_dmi_end_addr[<%=rn%>]);
...
//SCHEME2 (48)
 ...
 foreach(noncoh_regionsq[indx]) begin
    ...
    //dmi_ig_id = $urandom_range(0, $size(ncoreConfigInfo::intrlvgrp_vector)-1 );
  //dmi_ig_id = 1;
 ...
 foreach (nintrlv_grps[i]) begin
     ...
     foreach (csrq[j]) begin
       ...
       `uvm_info(get_name(),$psprintf("cust_svt_amba_system_configuration::%s: {0x%12h 0x%12h} domain_size=%0d rn_increment=%0d",  protocol_type, upp_addr, low_addr, domain_size, rn_increment),UVM_NONE)
          ...
          if(csrq[j].unit == ncoreConfigInfo::DII && onetime_dii_creation == 0) begin //{
           ...
           for(int k=0; k< <%=chiaiuIndexNum%>; k++) begin
            ...
            if(set_nonsnp[<%=rn%>] == 0) begin
              ...
              ncoreConfigInfo::dii_memory_domain_start_addr.push_back(rn_dii_low_addr[<%=rn%>]);
              ncoreConfigInfo::dii_memory_domain_end_addr.push_back(rn_dii_upp_addr[<%=rn%>]);
          ...
          if(csrq[j].unit == ncoreConfigInfo::DMI && onetime_dmi_creation == 0) begin //{
           ...
           for(int k=0; k< <%=chiaiuIndexNum%>; k++) begin
            ...
            if(set_innsnp[<%=rn%>] == 0) begin
              ...
              ncoreConfigInfo::dmi_memory_domain_start_addr.push_back(rn_dmi_low_addr[<%=rn%>]);
              ncoreConfigInfo::dmi_memory_domain_end_addr.push_back(rn_dmi_upp_addr[<%=rn%>]);
...
//SCHEME2 (48)
 ...
 if(i < ncoreConfigInfo::intrlvgrp_if.size)
 begin
     csrq = ncoreConfigInfo::get_memregions_assoc_ig(i);
