...
for(var j = 0; j < obj.DmiInfo.length; j++) { 
  if((obj.DmiInfo[j].fnNativeInterface == 'CHI-A')||(obj.DmiInfo[j].fnNativeInterface == 'CHI-B')) {
  // 
  } else { 
  dmi_range.push(j); //8,9,10,11...etc
 } }
...
var pidx = dmi_range[idx]; //extract by index
var dmi_prefix = obj.DmiInfo[pidx].strRtlNamePrefix;
var dmi_prefix_underscore_splitArr = [];
dmi_prefix_underscore_splitArr = dmi_prefix.split("_");
var dmi_prefix_underscore_cnt = dmi_prefix_underscore_splitArr.length;
console.log("fsys:connect_source2target_if: idx="+idx+",  pidx="+pidx+", fnNativeInterface="+obj.DmiInfo[pidx].fnNativeInterface+", digit_loc="+digit_loc+",  dmi_prefix="+dmi_prefix);
...
for(var port=0; port<obj.ariaObj.PortList.length; port++) {
  ...
  var fullSignalName   = fullDVSignal.split(".")[1]; 
   ...
   for(var temp=0; temp<dmi_prefix_underscore_cnt; temp++) {
    var str_temp;
    var underscore = '_';
    ...
    if(temp==0) {
      if(dmi_prefix_underscore_cnt>1) {
        finalInstName = str_temp.concat(underscore);
      } else {
        finalInstName = str_temp;
      }
    }
    else if(temp<(dmi_prefix_underscore_cnt-1)) {
  ...
  var obj1 = {};
  if(finalInstName === dmi_prefix && fullSignalName !==undefined ) {
  ...
  var drive_zero_to_snps_if_awid_msb = (obj.DmiInfo[pidx].interfaces.axiInt.params.wAwId<obj.DmiInfo[pidx].interfaces.axiInt.params.wArId)?1:0;
  var snps_if_awid_msb = obj.DmiInfo[pidx].interfaces.axiInt.params.wAwId;
  var snps_if_arid_msb = obj.DmiInfo[pidx].interfaces.axiInt.params.wArId;

  var drive_zero_to_snps_if_arid_msb = (obj.DmiInfo[pidx].interfaces.axiInt.params.wArId<obj.DmiInfo[pidx].interfaces.axiInt.params.wAwId)?1:0;
%>
