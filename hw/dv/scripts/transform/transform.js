...
const transformChiAiu = require('./chiaiu.js');
const transformIoAiu = require('./ioaiu.js');
const transformDii = require('./dii.js');
const transformDmi = require('./dmi.js');
const transformDce = require('./dce.js');
const transformDve = require('./dve.js');
const transformGiu = require('./giu.js');
const transformApbDebug = require('./apbdebug.js');
const lib = require('./lib.js');
const common = require('./common.js');
const perfCntUtil = require('../../common/lib_tb/perf_counter/perf_utils.js');
...
function sortInfoArray(arr) {
	if (!arr) return;
	arr.sort(   //order by nUnitId
...
function extractSmiWidths(config) {
	...
	smiObj.WNUNITID = config.system.wNUnitId;
...
function addInstanceMapAndHdlDefinesToChipletObj(fullDVJson, chipletObj) {
	const keysToIngore = ['strProjectName', 'assembly_name', 'assembly_id', 'chiplets', 'links', 'chipletRef'];
	for (const k of Object.keys(fullDVJson)) {
		const found = keysToIngore.find(val => val === k);
		if (!found) {
			if (chipletObj[k] === undefined) {
				chipletObj[k] = fullDVJson[k];
			}
		}
	}
}

function addTestBenchMapping(obj) {
	// obj.env_name = the runsim -e option
	switch (obj.env_name) {
		case 'fsys':
		case 'fsys_snps':
		case 'chi_subsys':
		case 'ioaiu_subsys':
		case 'ioaiu_subsys_snps':
			obj.testBench = 'fsys';
			break;
		case 'dii':
		case 'dii_snps':
		...
		case 'ioaiu':
		case 'ioaiu_snps':
			obj.testBench = 'io_aiu';
			break;
		case 'chi_aiu':
		case 'chi_aiu_snps':
			obj.testBench = 'chi_aiu';
			break;
		case 'dce':
			obj.testBench = 'dce';
			break;
		...
		case 'giu':
			...
			break;
		case 'cust_tb':
			obj.testBench = 'cust_tb';
			break;
		case 'emu_t':
			obj.testBench = 'emu';
			break;
		default: obj.testBench = obj.env_name;
	}
}

function convertAxiIntToArray(obj) {
    for (const aiu of obj.AiuInfo) {
        if((!aiu.fnNativeInterface.includes('CHI')) && aiu.nNativeInterfacePorts == 1) {
            if(obj.env_name == 'cust_tb') {
                aiu.interfaces.axiInt = [aiu.interfaces.axiInt];
            }
        }
    }
    return;
...
function transform(obj) {
	const uniqueDies = {};
	let isSingleDie = (obj.links && obj.links.length === 0);
	for (const dieInstance of obj.chiplets) {
		const baseChiplet = dieInstance.master_chiplet;
		if (uniqueDies[baseChiplet] !== undefined) continue;
		else {
			uniqueDies[baseChiplet] = true;
		}
		let dieObj;
		if (isSingleDie) {
			dieObj = obj.chipletRef[baseChiplet];
			addInstanceMapAndHdlDefinesToChipletObj(obj, dieObj);
		} else {
			dieObj = obj.chipletRef[baseChiplet].chipletRef['_root_'];
			addInstanceMapAndHdlDefinesToChipletObj(obj.chipletRef[baseChiplet], dieObj);
			addInstanceMapAndHdlDefinesToChipletObj(obj, dieObj);
		}
        //TODO: Check the below for loop with Allan
        // for(var idx = 0; idx < dieObj.AiuInfo.length; ++idx){    
        //     if((!dieObj.AiuInfo[idx].fnNativeInterface.includes('CHI')) && (dieObj.AiuInfo[idx].nNativeInterfacePorts == 1)){
        //         dieObj.AiuInfo[idx].interfaces.axiInt = [dieObj.AiuInfo[idx].interfaces.axiInt];
        //     }
        // }
		addTestBenchMapping(dieObj);

		sortInfoArray(dieObj.ChiaiuInfo);
		sortInfoArray(dieObj.IoaiuInfo);

		sortInfoArray(dieObj.DmiInfo);
		sortInfoArray(dieObj.DiiInfo);
		sortInfoArray(dieObj.DceInfo);
		sortInfoArray(dieObj.DveInfo);
		sortInfoArray(dieObj.GiuInfo);

		// Currently passing object references of dieObj.ChiaiuInfo and ...dieObj.IoaiuInfo in AiuInfo
		// TODO: Move to using Chi and Ioaiu instead of AiuInfo
		dieObj.AiuInfo = [...dieObj.ChiaiuInfo, ...dieObj.IoaiuInfo];

		dieObj.smiObj = extractSmiWidths(dieObj);
		dieObj.smiMsgObj = extractSmiMsgFields(dieObj.smiObj);

		// TODO: Clean up perfCntUtils. This is fragile code
		dieObj.perfCntUtil = { updateRetObj: perfCntUtil.updateRetObj };

		// TODO: This is redundant
		// {
		dieObj.nAIUs = dieObj.system.nAius;
		dieObj.nCHIs = dieObj.ChiaiuInfo.length;
		dieObj.nIOAIUs = dieObj.IoaiuInfo.length;
		dieObj.nDMIs = dieObj.system.nDmis;
		dieObj.nDIIs = dieObj.system.nDiis;
		dieObj.nDCEs = dieObj.system.nDces;
		dieObj.nDVEs = dieObj.system.nDves;
		dieObj.nGIUs = dieObj.system.nGius;
		//}
		
		const baseDieName = isSingleDie ? '' : baseChiplet;
		
		dieObj.ChiaiuInfo.forEach((o, idx) => {
			transformChiAiu(dieObj, o, baseDieName, idx);
		});
		dieObj.IoaiuInfo.forEach((o, idx) => {
			transformIoAiu(dieObj, o, baseDieName, idx);
		});
		dieObj.DiiInfo.forEach((o, idx) => {
			transformDii(dieObj, o, baseDieName, idx);
		});
		dieObj.DmiInfo.forEach((o, idx) => {
			transformDmi(dieObj, o, baseDieName, idx);
		});
		dieObj.DceInfo.forEach((o, idx) => {
			transformDce(dieObj, o, baseDieName, idx);
		});
		dieObj.DveInfo.forEach((o, idx) => {
			transformDve(dieObj, o, baseDieName, idx);
		});
		dieObj.GiuInfo.forEach((o, idx) => {
			transformGiu(dieObj, o, baseDieName, idx);
		});
		dieObj.DebugApbInfo.forEach((o, idx) => {
			transformApbDebug(dieObj, o, baseDieName, idx);
		});
        convertAxiIntToArray(dieObj);
	}
}
